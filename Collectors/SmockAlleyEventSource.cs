...
public sealed partial class SmockAlleyEventSource(IHttpClientFactory httpClientFactory) : IEventSource
{
    private const string TicketsolveFeedUrl = "https://smockalley.ticketsolve.com/shows.xml";

    public string Name => "smock-alley";
    public string Description => "Smock Alley Theatre Ticketsolve feed.";
    ...
    public async Task<EventSourceResult> CollectAsync(EventWindow window, CancellationToken cancellationToken)
    {
        ...
        {
            return EventSourceResult.Empty(Name, $"Smock Alley Ticketsolve feed returned {(int)response.StatusCode}.");
        }
    ...
    private IEnumerable<EventItem> ParseTicketsolveFeed(XDocument document, EventWindow window)
    {
        ...
        {
            var venueName = Text(venue.Element("name")) ?? "Smock Alley Theatre";
