...
public sealed class ThreeOlympiaTicketmasterEventSource(
    IHttpClientFactory httpClientFactory,
    IOptions<CollectorOptions> options) : IEventSource
{
    private const string VenueName = "3Olympia Theatre";

    public string Name => "3olympia";
    public string Description => "3Olympia Theatre events from the Ticketmaster Discovery API.";
    ...
    public async Task<EventSourceResult> CollectAsync(EventWindow window, CancellationToken cancellationToken)
    {
        ...
        {
            return EventSourceResult.Empty(Name, "Set Collector:TicketmasterApiKey or COLLECTOR__TICKETMASTERAPIKEY to enable 3Olympia via Ticketmaster.");
        }
        ...
        for (var page = 0; page < 5; page++)
        {
            ...
            {
                warnings.Add($"Ticketmaster returned {(int)response.StatusCode} for 3Olympia.");
                break;
            }
    ...
    private EventItem? ParseEvent(JsonElement item)
    {
        ...
        if (!string.Equals(venue, VenueName, StringComparison.OrdinalIgnoreCase))
        {
            return null;
        }
